...
class Plinth_Widget_RecentBlogPosts extends Plinth_Widget_Abstract {
  public function __construct() {
    parent::__construct(
      'plinth-recent-blog-posts-widget',
      'Plinth Recent Blog Posts Widget',
  ...
  public function widget($args, $instance, $echo = true) {
    ...
    $title = !empty($instance['title']) ? $instance['title'] : __('Recent Blog Posts');
    ...
    if($r->have_posts()) {
      ...
      while ($r->have_posts()) {
        ...
        $title = get_the_title();
        $id = get_the_ID();
        $out .= sprintf('<li><time class="post-date" datetime="%s">%s</time><a href="%s" title="%s">%s</a></li>',
