<?php
	class WpFastestCacheCreateCache extends WpFastestCache{
		...
		public function callback($buffer){
			...
			// $_SERVER["HTTPS"] = "off"
			// $_SERVER["HTTPS"] = ""
			if(preg_match("/^https/i", home_url()) && ((!isset($_SERVER["HTTPS"])) || ((isset($_SERVER["HTTPS"]) && $_SERVER["HTTPS"] != "on")))){
				return $buffer."<!-- Must be secure connection -->";
					...
					if(isset($_SERVER["HTTP_COOKIE"]) && $_SERVER["HTTP_COOKIE"]){
						if(preg_match("/qtrans/i", $_SERVER["HTTP_COOKIE"])){
							if(isset($_SERVER["REDIRECT_URL"]) && $_SERVER["REDIRECT_URL"]){
								$cachFilePath = $this->getWpContentDir()."/cache/all".$_SERVER["REDIRECT_URL"];
							}
						}
					}
				}

				$cachFilePath = rtrim($cachFilePath, "/")."/";
